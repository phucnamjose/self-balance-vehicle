% TUNE_CASCADE  Sweep balance-PID gains over the cascade sim (sim_cascade.m) and
% score each by steady-state tilt RMS, duty-saturation fraction, and drift, to
% find a clean (non-limit-cycle) balance. Prints the best few. Reuses the same
% firmware controller ports (balance_pid_sim / wheel_pi_sim).
clear; clc;
here = fileparts(mfilename('fullpath')); addpath(here);
p = params();

% Inner loop = the actual firmware wheel_pi gains (lumped L/R avg); the sim motor
% carries the real actuator lag p.motor.tau_e, so this matches the robot.
...
function [fell, rms_tilt, drift, sat] = run_one(p, inner, outer, BAL_MAX_TILT)
  ...
  nd=max(0,round(0.015/dt)); dq=zeros(1,nd+1);   % 15 ms lumped loop dead time
  randn('seed',1);
...
% Honest plant (motor lag + firmware inner loop): the ideal-motor optimum
% (Kp=18 Ki=750) limit-cycles just like the robot. Search broadly - Ki must clear
% g/r ~ 302, and with the inner-loop lag we expect to need MORE damping (Kp) and
% a LOWER Ki than the ideal sim.
Kps=[20 40 60 90];
Kds=[0 8 16];
Kis=[350 500 700 900];
best={}; rows=[];
...
% Prefer clean (unsaturated) balances: require satFrac < 0.05, then rank by a
% combined tilt+drift cost.
clean = ok(ok(:,5) < 0.05, :);
