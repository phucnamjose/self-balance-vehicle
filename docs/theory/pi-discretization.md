...
z-plane plots - why those choices barely change the design at 200 Hz.
...
- **Sampling** - the error is only known at the tick instants $t_k = k\,\Delta t$,
  with period $\Delta t = 1/f_s = 5$ ms at $f_s = 200$ Hz.
...
![Sampling and zero-order hold at 200 Hz](pi-disc-sampling.png)
...
differ - and why at 200 Hz it hardly matters.
...
- **Below ~100 rad/s (well past the loop crossover $\omega_c = 25.8$ rad/s):** all
...
- **Approaching the Nyquist frequency $\omega_N = \pi/\Delta t \approx 628$ rad/s:**
...
- **Heavy oversampling:** $\omega_c\,\Delta t = 25.8 \times 0.005 = 0.13$ rad - the
  loop is ~50x slower than the sample rate, deep in the region where the Bode
  curves above coincide.
- **Tiny per-tick step:** $K_i\,\Delta t \approx 0.77 \times 0.005 = 0.004$ per
...
- **The lag is already budgeted:** forward vs backward Euler differ by exactly
  **one sample** of delay, $\Delta t = 5$ ms. At $\omega_c$ that is
  $\omega_c\,\Delta t = 7.4^\circ$ of phase - and the tuning in
  [pi-tuning.md](pi-tuning.md) already lumps the ZOH + compute latency into a
  transport delay $T_d \approx 1.5\,\Delta t$ and still keeps $67^\circ$ of phase
  margin.

So at 200 Hz the choice of integration rule is a rounding error on the design; the
...
The measured wheel speed is noisy (encoder quantization at 200 Hz), so a
...
```octave
K = 34; tau = 0.19; dt = 1/200;          % nominal motor (illustration)
